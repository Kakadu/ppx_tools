...
let lam ?(label = Parr_simple) ?default pat exp =
  (* TODO: maybe change argument label: s/label/flag/ *)
  Exp.fun_ label default pat exp
let app f l = if l = [] then f else Exp.apply f (List.map (fun a -> Papp_simple, a) l)
